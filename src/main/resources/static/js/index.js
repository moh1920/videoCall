...
function handleLogout() {
    ...
    fetch(`http://localhost:9090/api/v1/user/logout?email=${email}`, {
        method: 'POST'
    })
        .then((response) => {
            return response;
...
function handleNewMeeting() {
    ...
    window.open(`vedeocall.html?username=${connectedUser.username}`, "_blank");
}
//
...
function handleJoinMeeting() {
    ...
    const url = `vedeocall.html?roomID=${roomId}&username=${connectedUser.username}`;
